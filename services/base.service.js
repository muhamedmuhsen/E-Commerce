...
class BaseService {
  async getAll(Model, query) {
    ...
    const [totalDocuments, documents] = await Promise.all([
      (apiFeatures.totalDocuments = await apiFeatures.mongooseQuery
        .clone()
        .countDocuments()),
    ...
    return documents
      ...
      : { documents: [], totalDocuments: 0, pagination };
  }
  ...
  async updateOne(Model, id, body) {
    ...
    if (body.image && Array.isArray(body.image)) {
      if (body.image.length > 6) {
        throw new BadRequestError("Max Images per product is 6");
      }
    }
