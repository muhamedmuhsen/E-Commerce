...
class CategoryService {
  ...
  async createCategory(data) {
    ...
    try {
      const category = await this.#BaseService.createOne(
        this.#CategoryModel,
        data
      );
  ...
  async updateCategory(id, data, image) {
    const category = await this.#BaseService.updateOne(
      this.#CategoryModel,
      id,
      data
    );
    if (!category) throw new NotFoundError("Category not found");
    if (image) category.image = image.filname;
    console.log(image);
